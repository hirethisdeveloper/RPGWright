...
`docs/` is the Playwright-style usage guide for people using RPGWright in their own project — a different audience from `agent_docs/` above (see `RPGWright.md` §12 for the split). All nine pages required for the v1.0 exit gate exist: `intro.md`, `writing-tests.md`, `configuration.md`, `cli.md`, `assertions.md`, `key-sequences.md`, `diagnostics.md`, `best-practices.md`, `ci.md`. Never link from a `docs/` page into `agent_docs/` or vice versa — they're for different readers.
